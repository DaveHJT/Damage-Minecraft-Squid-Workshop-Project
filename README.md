...
* In minecraft command system (mcfunction), we can use:
 ...
 3. If the damage score is 1, we uses poison effect, and tag the player a "poison_buffer" during the time that poison damage is not dealt yet. If another 1 damage arrives, we cancel the poison effect and buffer and add the 2x1 damage, so it can be dealt instantly. 
