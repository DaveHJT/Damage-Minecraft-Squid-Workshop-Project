...
* In minecraft command system (mcfunction), we can use:
 ...
 If the damage score is 1, we uses poison effect, and tag the player a "poison_buffer" when during the time that poison damage is not dealt yet. If another 1 damage arrives, we cancel the poison effect and buffer and add the 2x1 damage, so it can be dealt instantly. \
 Every tick we check the combination of damage score and the 1 damage pending in poison buffer, that wheter the damage left to dealt can kill the player, if so, we kill the player instantly using /kill commnad, and starts the [death message procedure](##Custom-Death-Message) and [pvp kills count procedure](##PVP-Kills-Count). So that all the kills by this damage system will always go through the kill procedure, the effects only hurts the player and never kills directly. \
